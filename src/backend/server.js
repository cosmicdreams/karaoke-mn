...
// Note: static public files are copied into dist by the build
...
async function fetchPreparedContent() {
  ...
  if (!url) {
    console.info('DRUPAL_CONTENT_URL not set; skipping prepared content fetch');
    return null;
  }
  try {
    ...
    console.warn('Could not fetch Drupal content; continuing without it:', err.message);
    return null;
  }
}
...
// Fetch session metadata by room code (for play view)
...
// Catch-all for client-side routes (exclude API and admin)
app.get(
  /^\/(?!api|auth|sessions|songs|search|preview|queue|phase2|assets|admin).*/,
  (req, res) => {
    res.sendFile(path.join(__dirname, '..', '..', 'dist', 'index.html'));
  }
);
